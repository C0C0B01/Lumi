import { lookupByName } from "@metro/common/wrappers";
import { React, ReactNative as RN } from "react-native";
...
const Status = lookupByName("Status");
...
export default function patchIcons(
	...
	if (config.biggerStatus) {
		patches.push(
			before("default", Status, (([props, ...rest]: [any, ...any[]], ...args: any) => [
				{
					...props,
	...
	if (plus.icons || plus.customOverlays || iconpack) {
		...
		patches.push(
			instead("render", RN.Image, (_args: string | any[], orig: (arg0: any) => any) => {
				const args = _args.slice();
				const [props] = args;

				if (props.ignore) return orig(args[0]);
				const { source } = props;
				...
				if (!asset?.httpServerLocation) return orig(args[0]);
